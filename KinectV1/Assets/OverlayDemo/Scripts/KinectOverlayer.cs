...
public class KinectOverlayer : MonoBehaviour
{
    ...
    void PlayerOne()
    {
        ...
        if (manager.IsJointTracked(depthViewer.userId, iJointIndex))
        {
            Vector3 posJoint = manager.GetRawSkeletonJointPos(depthViewer.userId, iJointIndex);
            ...
            if (posJoint != Vector3.zero)
            {
                ...
                if (debugText)
                {
                    debugText.GetComponent<GUIText>().text = "Tracked user ID: " + depthViewer.userId;  // new Vector2(scaleX, scaleY).ToString();
                }
